...
def for_ids(data):
    ...
    how_about_count=15
    global ids,ids_2,ids_3
    global space , case
    for data_ in data.transforms :
        ids=data_.fiducial_id
    global count , maximum_value
    #ids_2=False
    #ids_3=False
    #case = "None"
    space=np.append(space,ids)

    if len(space)==how_about_count:
        print(space)
        ...
        if ids_2 == False and ids_3 == False :

            case = "None"
        elif (ids_2 == True and ids_3 == False ) or (ids_2 == False and ids_3 == True):
            case = "One"
        elif ids_2 == True and ids_3 == True :
            case = "two"
        #print("2 is {}".format(ids_2))
        #print("3 is {}".format(ids_3))
        #print("case is " + case)
...
if __name__ == '__main__':
    try:
        rospy.init_node('my_bot_2_control')
        cmd_vel_pub = rospy.Publisher('tb3_1/cmd_vel', Twist, queue_size=10)
        odom_sub = rospy.Subscriber('tb3_1/odom', Odometry, odom_callback)
        scan_sub = rospy.Subscriber('tb3_1/scan', LaserScan, lader_callback)

        rospy.Subscriber("fiducial_transforms", FiducialTransformArray, for_ids)
        world_pose_sub = rospy.Subscriber('world_pose' , Pose , world_pose_)
        bot_1_pose_sub = rospy.Subscriber('bot_2_pose', Pose, bot_1_pose_)
        rospy.Subscriber("status",String,read_case)
